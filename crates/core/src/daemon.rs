    ...
    async fn handle_proc_event(
        ...
        match event {
            ProcEvent::Fork { parent_pid, child_pid, child_tgid } => {
                // Only count real process forks, not thread creation.
                // child_pid == child_tgid means the child is a new thread group leader
                // (a real fork). If child_pid != child_tgid, it's a new thread within
                // an existing process — don't count that toward fork-bomb detection.
                ...
                // Foreground inheritance — if parent is focused, child inherits.
                self.focus.on_fork(parent_pid, child_pid, &self.cgmgr).await;

                match ProcessInfo::from_pid(child_pid) {
                    Ok(info) => {
                        self.table.insert(info.clone());
                        // Skip normal classification if already boosted as foreground.
                        if !self.focus.is_foreground(child_pid) {
            ...
            ProcEvent::Exec { pid } => {
                match ProcessInfo::from_pid(pid) {
                    Ok(info) => {
                        self.table.insert(info.clone());
                        // After exec, re-evaluate even foreground pids (comm changed).
