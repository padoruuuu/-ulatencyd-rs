...
- `crates/core` (the `ulatencyd` daemon) — async server, via
...
(`generate_async: true` for the daemon and the shim, `false` — the default —
for the CLI), and each writes its generated `*.rs` into its own crate's
`OUT_DIR`. Because generation happens per-consumer against a filesystem
path rather than through a shared Rust dependency, the standalone
`system76-compat-shim`'s Cargo dependency graph never touches the main
workspace, and `cargo tree` run from the repo root never shows the shim or
anything it depends on (including `zbus`).
